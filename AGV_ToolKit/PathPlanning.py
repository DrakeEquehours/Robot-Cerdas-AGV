...
class PathPlanning:
    ...
    def setState(self, state):
        ...
        print("{} is the state".format(self.state[0]))
    ...
    def pathPlanningStep(self, a:str, status = False, update=False) -> str:
        """from KRAI_ToolKit.PathPlanning.PathPlanning

        Return a Path Plan of Robot
        ...
        Status : boolean, optional default=false
            Return array of plan if false
            Print a Planning Path


        Update : boolean, optional default=false
            Always update state after reach nodes if True
            Notes: Use if want to do path planning continously
        """
        for i in range(len(self.member)):
